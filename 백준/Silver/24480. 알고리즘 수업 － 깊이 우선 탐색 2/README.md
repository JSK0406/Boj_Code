...
que에 추가할 노드가 존재하는지 확인하고 추가하는 것보다 일단 추가하고 나중에 current로 왔을 때 visit여부를 체크하는 것이 훨씬 효율이 좋은 것 같다.
확인하는 것은 que 전체를 돌아야하지만 visit여부는 인덱싱으로 접근하기 때문이 아닐까 생각이 든다.

### 성능 요약

메모리: 68916 KB, 시간: 608 ms
